...
override IMAGE_NAME := breezeos-$(version)-$(edition)-$(KARCH)
...
$(IMAGE_NAME).iso: limine/limine kernel
	...
	cp -v kernel/bin-$(KARCH)/kernel.sys iso_root/boot/
...
ifeq ($(KARCH),x86_64)
	...
	mcopy -i $(IMAGE_NAME).hdd@@1M kernel/bin-$(KARCH)/kernel.sys ::/boot
